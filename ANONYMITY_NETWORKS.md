...
is made or until `lozzaxd` is shutdown and restarted with only public
connections enabled.
...
`--tx-proxy` which tells `lozzaxd` the ip/port of a socks proxy provided by a
...
which tells `lozzaxd` that ".onion" p2p addresses can be forwarded to a socks
...
`--anonymous-inbound`. This option tells `lozzaxd` the inbound address, network
...
which tells `lozzaxd` that a max of 25 inbound Tor connections are being
received at address "rveahdfho7wo4b2m.onion:28083" and forwarded to `lozzaxd`
localhost port 28083, and a default max I2P connections are being received at
address "cmeua5767mz2q5jsaelk2rxhf67agrwuetaso5dzbenyzwlbkg2q.b32.i2p:5000" and
forwarded to `lozzaxd` localhost port 30000.
...
`lozzaxd` RPC port - which is independent from the configuration for incoming
...
If a user only runs `lozzaxd` to send a transaction then quit, this can also
be used by an ISP to link a user to a transaction.

#### Mitigation

Run `lozzaxd` as often as possible to conceal when transactions are being sent.
...
`lozzaxd` currently selects two outgoing connections every 5 minutes for
...
`lozzaxd` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `lozzaxd` should likely disconnect
