...
class SNNetwork:
    def __init__(self, datadir, *, binpath='../../build/bin', sns=20, nodes=3):
        ...
        nodeopts = dict(lozzaxd=self.binpath+'/lozzaxd', datadir=datadir)
        ...
        vprint("Starting new lozzaxd service nodes with RPC on {} ports".format(self.sns[0].listen_ip), end="")
        for sn in self.sns:
            vprint(" {}".format(sn.rpc_port), end="", flush=True, timestamp=False)
            sn.start()
        vprint(timestamp=False)
        vprint("Starting new regular lozzaxd nodes with RPC on {} ports".format(self.nodes[0].listen_ip), end="")
        for d in self.nodes:
            vprint(" {}".format(d.rpc_port), end="", flush=True, timestamp=False)
            d.start()
        vprint(timestamp=False)

        vprint("Waiting for all lozzaxd's to get ready")
...
def chuck(net):
    ...
    hidden_node = Daemon(lozzaxd=net.binpath+'/lozzaxd', datadir=net.datadir)
    bridge_node = Daemon(lozzaxd=net.binpath+'/lozzaxd', datadir=net.datadir)
    for x in (4, 7):
        bridge_node.add_peer(net.all_nodes[x])
    bridge_node.add_peer(hidden_node)
    hidden_node.add_peer(bridge_node)

    vprint("Starting new chuck lozzaxd bridge node with RPC on {}:{}".format(bridge_node.listen_ip, bridge_node.rpc_port))
    bridge_node.start()
    bridge_node.wait_for_json_rpc("get_info")
    net.sync(extra_nodes=[bridge_node], extra_wallets=[chuck])

    vprint("Starting new chuck lozzaxd hidden node with RPC on {}:{}".format(hidden_node.listen_ip, hidden_node.rpc_port))
    hidden_node.start()
